...
public class ChatHub : Hub
{
  ...
  {
    IClientProxy proxy;
    if (Users.ContainsKey(message.From))
    {
            UserModel user = Users[message.From];
            if (string.IsNullOrEmpty(message.To))
            {
                message = new()
                {
                    ...
                    To = message.From,
                    Body = string.Format(
                            "Please Set a spicific User or Group")
                };
                proxy = Clients.Client(user.ConnectionId);
                await proxy.SendAsync("ReceiveMessage", message);
            }
                ...
                else // Assume the item is a group name to send the message to.
                {
                    if (string.IsNullOrEmpty(user.Groups))
                    {
                        message = new()
                        {
                            ...
                            To = message.From,
                            Body = string.Format(
                            "You r not in {0} group", arg0: userOrGroup)
                        };
                        proxy = Clients.Client(user.ConnectionId);
                        await proxy.SendAsync("ReceiveMessage", message);
                    }
                    ...
                    {
                        if (user.Groups.Contains(userOrGroup))
                        {
    ...
    else
    {
        message = new()
        {
            ...
            To = message.From,
            Body = string.Format(
    "Please Registe yourself first")
        };
        proxy = Clients.Client(Context.ConnectionId);
        await proxy.SendAsync("ReceiveMessage", message);
    }
  }
}
